...
/obj/item/projectile/proc/attack_mob(var/mob/living/target_mob, var/distance, var/miss_modifier=0)
	if(!istype(target_mob))
		flog("Projectile [src] failed to attack living mob [target_mob]")
		return

	flog("Projectile [src] began to attack living mob [target_mob]")
	//roll to-hit
	miss_modifier = max(15*(distance-2) - round(15*accuracy) + miss_modifier, 0)

	flog("miss_modifier: [miss_modifier]")

	flog("defense zone of projectile is [def_zone], target_mob is [target_mob]")

	var/hit_zone = get_zone_with_miss_chance(def_zone, target_mob, miss_modifier, ranged_attack=(distance > 1 || original != target_mob)) //if the projectile hits a target we weren't originally aiming at then retain the chance to miss
	flog("Hit zone in [src].attack_mob([target_mob],[distance],[miss_modifier]) == [hit_zone]")
	var/result = PROJECTILE_FORCE_MISS
	flog("Default result: [result]")

	if(hit_zone)
		flog("There is a hit zone in projectile.attack_mob()")
		...
		flog("Result after hit zone: [result]")

	if(result == PROJECTILE_FORCE_MISS)
		flog("projectile missed")
		if(!silenced)
...
/obj/item/projectile/Bump(atom/A as mob|obj|turf|area, forced=0)
	...
	if(ismob(A))
		...
		if(istype(A, /mob/living))
			...
			flog("projectile [src] attempted to bump living mob [A]")
