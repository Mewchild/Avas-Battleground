...
//	var/finishing_halting = 0 // did we halt recently? don't play movement sounds
	var/velocity = 2.0 // tiles moved every move: currently, we need to move 270 or so tiles
				...
				if (!first_car || first_car == tcc)
					for (var/obj/train_pseudoturf/tpt in tcc.backwards_pseudoturfs)
						if (tpt.y + getMoveInc() + 1 >= starting_point.y)
							flog("FAILED can_move_check() while going backwards!", "train-debug")
							return 0
	return 1
...
/datum/train_controller/proc/sound_loop()
	...
	if (last_played["halting"] == -1 || world.time - last_played["halting"] >= halting_sound_delay)
		if (coming_to_halt() && playing == "")
			...
			play_halting_sound()
			flog("PLAYED halting sound", "train-sound-debug")
			playing = "halting"
	...
	if (last_played["movement"] == -1 || world.time - last_played["movement"] >= movement_sound_delay)
		if (moving && can_move_check() && playing == "")
			...
			play_movement_sound()
			flog("PLAYED movement sound", "train-sound-debug")
			playing = "movement"
		...
		if (istype(m) && m.client)
			...
			if (tcc && istype(tcc))
				...
				if (!locate(/obj/train_connector) in get_turf(m))
					...
					flog("PLAYED quiet sound [sound] to [m] w client [m.client]. volume = [volume]", "train-sound-debug")
				else
					m.client.playsound_personal(sound, volume)
					flog("PLAYED loud sound [sound] to [m] w client [m.client]. volume = [volume] and should == 100", "train-sound-debug")
			else
				flog("would have played sound [sound], but couldn't find a tcc.", "train-sound-debug")
