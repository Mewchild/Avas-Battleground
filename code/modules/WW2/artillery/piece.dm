	...
	if (!user.lying)
		...
		if (href_list["fire"])
			if ("fire")
				...
				if (istype(user_area, /area/prishtina/german_base_outside))
					user << failure_msg
					return
				if (istype(user_area, /area/prishtina/german_base))
					user << failure_msg
					return
...
/obj/machinery/artillery/tube
	...
	proc/fire(var/x, var/y, shell)
		...
		var/abs_dist = abs(t.x - other.x) + (t.y - other.y)

		travel_time = (round(abs_dist/50) * 10) + 20 // must be at least 2 seconds for the incoming sound to
		// work right
